...
// /delete-account — account-deletion request (URL inventory: keep; the JB app's stores listing requires this URL).
// Same contract as /login: real form UI, nothing transmitted until the
// Dutchie account API is wired. Auth/utility route: noindex + robots.txt
// disallow (unlinked from public chrome), not in sitemap.

export const metadata: Metadata = {
  title: 'Delete Account',
  description: 'Request deletion of your Jungle Boys account.',
    ...
    <main data-nav-theme="dark" className="bg-[var(--color-background)] px-2 pb-24 pt-2 text-[var(--color-foreground)] md:px-3">
      ...
      <div className="relative overflow-hidden rounded-[1.75rem] bg-[#0b0b0b] px-6 pb-16 pt-24 text-white md:rounded-[2.5rem] md:pb-20 md:pt-32">
        ...
        <div className="relative mx-auto max-w-xl text-center">
          ...
          <h1 className="font-display mt-4 uppercase leading-[0.85]" style={{ fontSize: 'min(11vw, 4.5rem)' }}>
            Delete account
          </h1>
          <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-white/70" style={{ fontFamily: 'var(--font-brand)' }}>
            This removes your account, order history and PWF Rewards points. It cannot be undone.
          </p>
          <div className="mt-8">
            <AuthStubForm kind="delete" />
          </div>
        </div>
      </div>
      </Reveal>
