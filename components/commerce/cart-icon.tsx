  ...
  return (
    <span className="relative inline-flex h-12 w-12 items-center justify-center">
      {CART_ICON_SRC ? (
        ...
        <img src={CART_ICON_SRC} alt="" className="h-12 w-12 object-contain invert" />
      ) : (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" className="h-12 w-12" aria-hidden>
      ...
      <span
        data-cart-count
        className={`absolute ${COUNT_POS} flex -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full text-[13px] leading-none transition-[background-color,opacity] duration-200 ${
          count > 0
            ? 'h-5 w-5 bg-[var(--color-accent)] text-black'
            : CART_ICON_SRC
              ? // the custom art carries its own badge circle, white in the
                ...
                'h-[0.8rem] w-[0.8rem] bg-white text-transparent'
              : 'h-5 w-5 border border-white/30 text-white/50'
        }`}
